...
@section('main')
	@include('app.shared.breadcrumbs',['crumbs' => [
		...
		[
			'name' => __('BIN Check API Settings'),
			'url' => '#',
		],
	]])
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-md-8 col-lg-9 col-xl-10">
				<h1>{{ __('BIN Check API Settings') }}</h1>
				<!--<form action="{{ route('save-settings') }}" method="POST" autocomplete="off" class="mb-3">
					...
					<div class="card bg-dark text-white">
						...
						<div class="card-footer">
							<input type="submit" class="btn btn-light" value="{{ __('Save BIN Check Settings') }}" />
						</div>
					</div>
				</form> -->
				<div class="alert alert-info">{{ __('Nothing to Configure at this time') }}</div>
			</div>
		</div>
	</div>
@endsection